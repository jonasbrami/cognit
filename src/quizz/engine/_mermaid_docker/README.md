...
**Built lazily.** `quizz` runs `docker build` the first time it needs the
...
```bash
docker build -t quizz-mermaid-validator:local src/quizz/engine/_mermaid_docker/
```
...
```bash
echo 'flowchart LR
A --> B' | docker run --rm -i quizz-mermaid-validator:local
```
