...
def test_agent_generates_quiz_from_diff(tmp_path: Path):
    ...
    mcp_cfg.write_text(
        json.dumps(
            {"mcpServers": {"cognit": {"command": sys.executable, "args": ["-m", "cognit.mcp"]}}}
        )
    )
    ...
    env = {
        **os.environ,
        "COGNIT_PR_URL": "DUMMY",
        "COGNIT_PR_NUMBER": "0",
        "COGNIT_HTTP_PORT": str(_free_port()),
        "COGNIT_SNAPSHOT_PATH": str(snapshot),
        "COGNIT_REPO_ROOT": repo,
    }
    diff = subprocess.run(
        ["git", "diff", "main...HEAD", "--", "src/cognit/mcp/state.py"],
        capture_output=True,
        text=True,
        cwd=repo,
    ).stdout[:6000]
    kickoff = (
        "Build a 2-question quiz about this diff and render it with set_quiz; do NOT "
    ...
    assert snapshot.exists(), (
        f"no snapshot written. claude stdout:\n{proc.stdout[-2000:]}\nstderr:\n{proc.stderr[-1000:]}"
    )
